...
namespace ServerLibrary.Repositories.Implementations
{
    public class SanctionRepository(AppDbContext appDbContext) : IGenericRepositoryInterface<Sanction>
    {
        ...
        await appDbContext.Sanctions.AsNoTracking().Include(x=>x.SanctionType).ToListAsync();

        public async Task<Sanction> GetById(int id)=> await appDbContext.Sanctions.FirstOrDefaultAsync(x => x.EmployeeId == id);
        ...
        {
            var obj = await appDbContext.Sanctions.FirstOrDefaultAsync(x => x.EmployeeId == item.EmployeeId);
